...
import { activeElements, select } from './settings.js';
const app = {
  initData: function () {
    const thisApp = this;
  ...
  activatePage: function (pageId) {
    ...
    for (let page of thisApp.sides) {
      if (page.id == pageId) {
        page.classList.add(activeElements.sides.active);
      } else {
        page.classList.remove(activeElements.sides.active);
      }
    }
  ...
  initContact: function () {
    const thisApp = this;
    const newElement = document.querySelector('#contact');
    thisApp.newContact = new Contact(newElement);
  },
